...
namespace Framework.Domain.Interfaces.Repositories
{
    public interface IGenericQueryRepositoryWithNonRelation<T, U> where T : IBaseEntity<U>
    {
        ...
        Task<IEnumerable<T>> WhereAsync(Expression<Func<T, bool>> selector, bool includeLogicalDeleted = false, Pagination? pagination = null, ICollection<Sort>? sorts = null, CancellationToken cancellationToken = default);
        Task<IEnumerable<T>> GetAllAsync(bool includeLogicalDeleted = false, Pagination? pagination = null, ICollection<Sort>? sorts = null, CancellationToken cancellationToken = default);
